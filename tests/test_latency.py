...
from lib.bot import ZoomRTMSBot, CHUNK_DURATION, CHUNK_SIZE

JWT_SECRET_KEY = os.getenv("JWT_SECRET_KEY")
VIEWER_URL = "ws://localhost:8000/ws/view"
...
@pytest.mark.asyncio
async def test_cold_vs_hot_latency():
    """
    Compare Cold vs Hot latency.
    Fix: Streams silence between utterances to force VAD to close Utterance 1.
    """
    bot = ZoomRTMSBot()
    await bot.connect()
    ...
    viewer_url = f"{VIEWER_URL}/zoom/{session_id}?language=en&token={query_token}"
    ...
    async with aiohttp.ClientSession(cookies={"app_auth_token": cookie_token}) as session:
        async with session.ws_connect(viewer_url) as viewer_ws:
            ...
            print(f"\n==========================================")
            print(f" LATENCY COMPARISON (Cold vs Hot)")
            print(f"==========================================")
